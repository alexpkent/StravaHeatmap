...
## CDN (Optional but improves load times)
...
1. F5/'npm start' in VsCode to start the Azure Function App for the api.
   - There is a `local.settings.json` file that contains the Cofig settings specified above in Deployment Settings. Note: Connection String has to be prefixed for local settings so called `AzureWebJobsBLOB_CONNECTION_STRING`
